...
- Swipe right from the **left edge** while browsing: the rounded sidebar follows your finger, with gentle resistance at its ends and a spring on release. Drag left to close it, or reverse direction before letting go. Inside a course or lesson, the same swipe goes back one screen. The menu button opens the sidebar directly, with Home, current-language Library, Words, Review, Account, and appearance controls. Close it with the X, an outside tap, or a leftward swipe.
